...
LATEST_YEAR <- as.POSIXlt(Sys.Date())$year + 1900L - 1L
TEN_YEARS_AGO <- LATEST_YEAR - 10L
```
...
1. Identify the groups and rows that contain the data you need
   * Find potential groups by searching the `Universe` and `Description` fields
     of `hercacstables::ACS_GROUP_METADATA`.
   * Identify specific variables with `hercacstables::ACS_VARIABLE_METADATA`:
...
One way to do this is to search `hercacstables::ACS_GROUP_METADATA` for groups
