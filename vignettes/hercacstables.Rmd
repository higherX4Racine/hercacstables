...
1. Identify the groups and rows that contain the data you need
   * Find potential groups by searching the `Universe` and `Description` fields
     of `hercacstables::GLOSSARY_OF_ACS_GROUPS`.
   * Identify specific variables with `hercacstables::GLOSSARY_OF_ACS_VARIABLES`:
...
2. Define the geographies that you will use
   * Use `hercacstables::GLOSSARY_OF_ACS_GEOGRAPHIES` for this, too!
...
One way to do this is to search `hercacstables::GLOSSARY_OF_ACS_GROUPS` for groups
...
household_variables <- hercacstables::GLOSSARY_OF_ACS_VARIABLES |>
    dplyr::filter(
        .data$Dataset == "ACS1",
