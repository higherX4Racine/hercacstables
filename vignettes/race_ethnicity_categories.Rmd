...
The `hercacstables` package has many tools and glossary tables that should make
...
broad_subtables <- hercacstables::GLOSSARY_OF_ACS_GROUPS |>
...
Since the case is so common, `hercacstables` describes it with the glossary
...
tables_that_name_race <- hercacstables::GLOSSARY_OF_ACS_GROUPS |>
    dplyr::filter(
    ...
    dplyr::left_join(
        dplyr::count(hercacstables::GLOSSARY_OF_ACS_VARIABLES,
                     .data$Group,
                     name = "rows"),
        by = "Group"
    )
...
`hercacstables::RACE_ETHNICITY_SUBTABLES` and map them onto rows in
`B03002`.
...
racine_race_ethnicity_variables <- RACE_ETHNICITY_SUBTABLES |>
    dplyr::filter(
