...
LATEST_YEAR <- as.POSIXlt(Sys.Date())$year + 1900L
```
...
broad_subtables <- hercacstables::ACS_GROUP_METADATA |>
...
table `hercacstables::RACE_ETHNICITY_SUBTABLE_METADATA`.
...
tables_that_name_race <- hercacstables::ACS_GROUP_METADATA |>
    dplyr::filter(
    ...
    dplyr::left_join(
        dplyr::count(hercacstables::ACS_VARIABLE_METADATA,
                     .data$Group,
                     name = "rows"),
        by = "Group"
    )
...
`hercacstables::RACE_ETHNICITY_SUBTABLE_METADATA` and map them onto rows in
`B03002`.
...
Next, we pull `r LATEST_YEAR - 2005 - 1` years of ACS data.
